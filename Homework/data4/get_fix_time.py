...
class log():
    ...
    def __init__(self,verran):
        ...
        """
        self.verran = verran
        self.repo = "/Users/hurryzhao/linux-stable"
        self.bug_time = {}
        self.bug_timediff = {}
...
def main():
    file = open("/Users/hurryzhao/linux-stable/version.txt")
    for line in file.readlines():
        line=line.strip('\n')
        print(line)
        b = log(line).get_commit()
        file_name = str(line + '.csv')
        pd.DataFrame.from_dict(data=b, orient='index').to_csv(file_name, header=False)
    file.close()
